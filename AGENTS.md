...
### R*-tree node layout
Nodes are `alignas(64)` structs (one cache line). The union between `children` (internal) and `values` (leaf) is disambiguated by the `is_leaf` flag. `MaxChildren` is a compile-time parameter (default 9); `MinChildren = MaxChildren / 2`. The pool allocator keeps nodes contiguous in memory. Leaf value entries store small values inline, but values larger than `rtree_inline_value_max_size` (128 bytes) are boxed behind a `unique_ptr` (`RTreeValueEntry`'s `Boxed` specialization), so large payloads don't inflate node storage — and, because leaf and child storage share the union, don't inflate internal nodes either. Access the stored value through the entry's `value()` accessor, not a field.
...
- Update PLAN.md with any changes. Keep completed items to a single terse line (what was done, where it lives) — no multi-line implementation descriptions. PLAN.md tracks remaining work, so detail belongs only on pending items.
